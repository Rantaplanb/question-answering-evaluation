...
print("Script starts...")
output_file = '../resources/csv_files/questions_with_answers_from_all_models_with_helsinki_translator.csv'
...
for subject in data:
    ...
    for QnA in paragraphs:      
        en_context = translate(QnA['context'], 'helsinki', 'el', 'en')
        print('Context:', en_context)
        for qna in QnA["qas"]:
            en_question = translate(qna["question"], 'helsinki', 'el', 'en')
            print('Question: ', en_question)
            results = []
            question = en_question
            for i in range(len(models)):
                result, elapsed_time = answer_question(en_context, en_question, models[i])
                results.append([question, models[i], elapsed_time, result['score'], result['start'], result['end'], result['answer'], translate(result['answer'], 'helsinki', src='en', dest='el'), qna['answers'][0]['text']])
                question = ""
            with open(output_file, 'a', encoding='UTF16') as file:
                writer = csv.writer(file)
